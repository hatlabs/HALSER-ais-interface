...
// Type 24 Part A: MMSI 271041815, name contains "PROGUY"
void test_decode_type24_part_a() {
  const char* payload = "H42O55i18tMET4@T4@Dn0000000";
  auto result = ais::decode_payload(payload, 2);
  TEST_ASSERT_TRUE_MESSAGE(result.has_value(), "decode should succeed");

  auto* data = std::get_if<ais::ClassBStaticData>(&result.value());
  TEST_ASSERT_NOT_NULL_MESSAGE(data, "should be ClassBStaticData");

  TEST_ASSERT_EQUAL_UINT8(24, data->message_id);
  TEST_ASSERT_EQUAL_UINT32(271041815, data->mmsi);
  TEST_ASSERT_EQUAL_UINT8(0, data->part_number);
  // Part A zeroes Part B fields
  TEST_ASSERT_EQUAL_UINT8(0, data->ship_type);
  TEST_ASSERT_EQUAL_UINT32(0, data->mothership_mmsi);
}

// Type 24 Part B: MMSI 271041815, ship_type 60, callsign TC6163
// Normal craft: dimensions extracted, mothership_mmsi = 0
void test_decode_type24_part_b_normal() {
  const char* payload = "H42O55lti4hhhilD3nink000?050";
  auto result = ais::decode_payload(payload, 0);
  TEST_ASSERT_TRUE_MESSAGE(result.has_value(), "decode should succeed");
  ...
  TEST_ASSERT_EQUAL_UINT8(24, data->message_id);
  TEST_ASSERT_EQUAL_UINT32(271041815, data->mmsi);
  TEST_ASSERT_EQUAL_UINT8(1, data->part_number);
  TEST_ASSERT_EQUAL_UINT8(60, data->ship_type);
  TEST_ASSERT_EQUAL_STRING("TC6163", data->callsign);
  TEST_ASSERT_EQUAL_UINT16(0, data->to_bow);
  TEST_ASSERT_EQUAL_UINT16(15, data->to_stern);
  TEST_ASSERT_EQUAL_UINT16(0, data->to_port);
  TEST_ASSERT_EQUAL_UINT16(5, data->to_starboard);
  TEST_ASSERT_EQUAL_UINT32(0, data->mothership_mmsi);
}

// Verify auxiliary craft (MMSI prefix 98) branches correctly
void test_decode_type24_part_b_auxiliary() {
  // MMSI 981234567 / 10000000 = 98 → is_auxiliary = true
  // MMSI 271041815 / 10000000 = 27 → is_auxiliary = false
  TEST_ASSERT_EQUAL(98, 981234567u / 10000000);
  TEST_ASSERT_EQUAL(27, 271041815u / 10000000);
}
...
int main() {
  ...
  // Type 24 decoding
  RUN_TEST(test_decode_type24_part_a);
  RUN_TEST(test_decode_type24_part_b_normal);
  RUN_TEST(test_decode_type24_part_b_auxiliary);
