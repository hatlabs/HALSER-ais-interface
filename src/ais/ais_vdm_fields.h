...
// num_fields: number of fields (must be >= 7)
// reassembler: multi-part reassembly state
// now_ms: current time in milliseconds (for reassembly timeout)
//
// SentenceParser keeps the sentence address ("!AIVDM"/"!AIVDO") in field 0, so
// the VDM data fields start at index 1:
//   [0] address  [1] total_fragments  [2] fragment_num  [3] seq_id
//   [4] channel  [5] payload          [6] fill_bits
//
// Returns:
...
inline VDMParseResult parse_vdm_fields(const char* field_strings,
                                        ...
                                        uint32_t now_ms) {
  if (num_fields < 7) return std::nullopt;

  int total_fragments = atoi(field_strings + field_offsets[1]);
  int fragment_num = atoi(field_strings + field_offsets[2]);
  const char* seq_id_str = field_strings + field_offsets[3];
  int seq_id = seq_id_str[0] != '\0' ? atoi(seq_id_str) : 0;
  // field 4 = channel (not used for decoding)
  const char* payload = field_strings + field_offsets[5];
  int fill_bits = atoi(field_strings + field_offsets[6]);
